...
namespace AtomApp
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            elements = CollectionAdder(elements);
            CollectionSeer(elements);
            FirstSerializer(elements);
            Atom fileAtom = FirstDeSerializer();
            CollectionSerializer(elements);
            List<Atom> fileAtoms = CollectionDeSerializer();
        }
        
        static List<Atom> CollectionAdder(List<Atom> elements)
        {
        ...
        static void CollectionSeer(List<Atom> elements) //1 Display all
        {
        ...
        static void FirstSerializer(List<Atom> elements) //2 Serialize First
        {
            string filename = "Atoms.json";
        ...
        static Atom FirstDeSerializer() //3 Deserialize and display
        {
            string filename = "Atoms.json";
            TextReader reader = new StreamReader(filename);
            Atom fileAtom = JsonSerializer.Deserialize<Atom>(reader.ReadToEnd());
            Console.WriteLine("File read from JSON:");
            Console.WriteLine($"{fileAtom.ToString()}");
            reader.Close();
            Console.WriteLine("");
            return fileAtom;
        }

        static void CollectionSerializer(List<Atom> elements) //4 Serialize Collection
        {
            string filename = "Atoms.json";
        ...
        static List<Atom> CollectionDeSerializer() //3 Deserialize and display
        {
            string filename = "Atoms.json";
            TextReader reader = new StreamReader(filename);
            List<Atom> fileAtoms = JsonSerializer.Deserialize<List<Atom>>(reader.ReadToEnd());
            Console.WriteLine("Collection read from JSON:");
            CollectionSeer(fileAtoms);
            reader.Close();
            Console.WriteLine("");
            return fileAtoms;
        }
